...
def deploy():
    """
    archive a file and and deploy it on a server
    args:
        No arguement
    returns:
        The value of do de_deploy if a valid path was given
        else false
    """
    path = do_pack()
    if path:
...
def do_clean(number):
    """
    Delete all the archives in the /versions folder
    Delete all archive in the web release
    args:
